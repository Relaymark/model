...
// before sending to server
  let instanceKeywords = ['$$array', '$save', '$destroy',
...
// https://github.com/angular/angular.js/blob/master/src/ngResource/resource.js#L30
  let shallowClearAndCopy = function (src, dst) {
    dst = dst || {};
  ...
  let getFactoryFn = function() {
    return function ($rootScope, $http, $q, $log, $cacheFactory) {
      ...
      function modelFactory(url, options) {
        ...
        // as it is
        let wrapAsNewModelInstance = function (rawObj, arrayInst) {
          // create an instance
        ...
        function Model(value, ignorePkMapping) {
          ...
          instance.$saveWithUpload = function (filesObject) { //filesObject is a key-value ( key == propertyName / value == files array )
            ...
            promise.then(function (returnedValue) {
              ...
              let broadcastName = actionType === 'post' ? 'created' : 'updated';
         ...
         */
        let existingCache = $cacheFactory.get(url);
        ...
        Model.$buildRequest = function (action, param, data, extras) {
          ...
          if (!clone.override) {
            ...
            //  ==> or this shortcut : RegistrationModel.sendRegistrationEmail({objectToPost},myId)
              ...
              else if (angular.isString(extras) || angular.isNumber(extras)) { // if we want to pass shortId directly and it's the only param. in URI
              ...
              else if (angular.isString(data) || angular.isNumber(data)) { // if we want to pass shortId directly and it's the only param. in URI
            ...
            if (action === 'get' || action === 'post' || action === 'update' || action === 'delete'
              || action === 'postWithUpload' || action === 'updateWithUpload') {
        ...
        Model.$call = function (params) {
          ...
          promise.then(function (response) {
            ...
            if (params.afterRequest) {
              let transform = params.afterRequest(response.data);
              if (transform) {
